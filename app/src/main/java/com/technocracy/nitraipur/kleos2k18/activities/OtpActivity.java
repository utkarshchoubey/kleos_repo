...
public class OtpActivity extends AppCompatActivity {
    ...
    public void otp(View view) {
        ...
        if(String.valueOf(otpEdit.getText()).length() == 4){
        indicatorView.show();
        mExplosionField.explode(view);
        mExplosionField.explode(resend);
        mCvCountdownView.stop();
//        String username = userPreferences.getUsername();
//        int otp = Integer.parseInt(otpEdit.getText().toString());
//            final User user = new User(username,otp);
//            Call<User> call = apiBase.otpVerification(user);
//            call.enqueue(new Callback<User>() {
//                @Override
//                public void onResponse(Call<User> call, Response<User> response) {
//                    if(response.isSuccessful()){
//                        user.setMessage(String.valueOf(response.body()));
//                        if(user.getMessage().equals("Otp Verified") ) {
